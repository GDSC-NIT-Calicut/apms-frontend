...
const Cards: React.FC<CardProps> = ({ title, date, status }) => {
  return (
    <div className="border border-white/40 w-full max-w-md px-6 py-6 flex flex-col gap-4 bg-black text-white shadow-sm">
      <div className="text-base font-medium">{title}</div>
      <div className="flex justify-between items-center flex-wrap gap-y-2">
        <span className="text-sm text-gray-300">{date}</span>
        <span
          className={`text-sm px-6 py-3 font-semibold cursor-pointer ${statusStyles[status]}`}
        >
          {status}
        </span>
      </div>
